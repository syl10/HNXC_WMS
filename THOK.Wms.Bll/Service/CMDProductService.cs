      ...
      //  DownProductBll Product = new DownProductBll();
        ...
        public object GetDetails(int page, int rows, string ProductName, string ProductCode, string CustomCode, string BrandCode, string UniformCode, string AbcTypeCode, string ShortCode, string PriceLevelCode, string SupplierCode)
        {
           IQueryable<CMD_PRODUCT> ProductQuery = ProductRepository.GetQueryable();
            var product = ProductQuery.Where(c => c.PRODUCT_NAME.Contains(ProductName)
                && c.PRODUCT_CODE.Contains(ProductCode)
                //&& c.BrandCode.Contains(BrandCode)
                //&& c.UniformCode.Contains(UniformCode)
                //&& c.SupplierCode.Contains(SupplierCode)
                )
                .OrderBy(c => c.PRODUCT_CODE)
                .Select(c => c);
        ...
        //{
        //   IQueryable<CMD_PRODUCT>ProductQuery = ProductRepository.GetQueryable();
        //    var product = ProductQuery.OrderBy(p => p.PRODUCT_CODE).Where(p => p.PRODUCT_CODE == p.PRODUCT_CODE);

        //    if (QueryString == "PRODUCT_CODE")
        //    {
        ...
        //        //product = product.Where(p => storages.Any(s => s == p.PRODUCT_CODE));

        //        product = product.Where(p =>p.PRODUCT_CODE));
        //    }
        ...
        {
           IQueryable<CMD_PRODUCT>ProductQuery = ProductRepository.GetQueryable();
            var product = ProductQuery.OrderBy(p => p.PRODUCT_CODE).Select(p => new { p.PRODUCT_CODE, p.PRODUCT_NAME });
            int total = product.Count();
            product = product.Skip((page - 1) * rows).Take(rows);
        ...
        public object GetProductBy(int page, int rows, string QueryString, string Value)
        {
           ...
           IQueryable<CMD_PRODUCT>ProductQuery = ProductRepository.GetQueryable();
            var product = ProductQuery.Where(c => c.PRODUCT_CODE.Contains(productCode) && c.PRODUCT_NAME.Contains(productName))
                .OrderBy(c => c.PRODUCT_CODE)
                .Select(c => new { 
                    c.PRODUCT_CODE,
                    c.PRODUCT_NAME
        ...
        public System.Data.DataTable GetProduct(int page, int rows, string ProductName, string ProductCode, string CustomCode, string BrandCode, string UniformCode, string AbcTypeCode, string ShortCode, string PriceLevelCode, string SupplierCode)
        {
           IQueryable<CMD_PRODUCT>ProductQuery = ProductRepository.GetQueryable();
            ...
            dt.Columns.Add("产品条码", typeof(string));
            dt.Columns.Add("产品名称", typeof(string));
            dt.Columns.Add("原产地", typeof(string));
            dt.Columns.Add("年份", typeof(string));
            dt.Columns.Add("等级", typeof(string));
            dt.Columns.Add("形态", typeof(string));
