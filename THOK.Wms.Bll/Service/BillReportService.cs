...
namespace THOK.Wms.Bll.Service
{
    class BillReportService : ServiceBase<BILLREPORT >, IBillReportService
    {
        ...
        //入库单
        public bool  StockinPrint(string Path, string username, string PrintCount, string BILLNO, string BILLDATEFROM, string BILLDATETO, string BTYPECODE, string BILLMETHOD, string STATE, string CIGARETTECODE, string FORMULACODE)
        {
            try
            {
                IQueryable<BILLREPORT> query = BillReportRepository.GetQueryable();
                var stockin = query.Select(i => new {
                    i.MBILL_NO,  //单号
                    i.BILL_DATE, //日期
                    i.BATCH_WEIGHT, //批次重量
                    i.BILL_METHOD, //单据方式
